...
1. LA PREMIÈRE RUN — lis le transcript qui t'est assigné, écran par écran,
   ...
   /tmp/panel2/paquet/transcripts/v1.128.3-premiere-run-curieuse.md
...
2. UNE VIE SUIVANTE — joue UNE vie complète sur la table Python, depuis
   ...
   cd /tmp/panel2/table1 && python3 pactum.py nouvelle
   ...
   joueur, pas au hasard : tu es CURIEUX : tu regardes, tu parles aux gens, tu observes avant d'agir, tu évites le dé quand une option sans dé existe.
...
3. IMAGES (facultatif, seulement si un texte te fait douter) :
   ...
   ⚠️ Tâche en plus pour toi : ouvre l'image de l'écran `campement` (le Moulin sans Ailes) et dis, image ouverte, si le moulin a des ailes ou non. C'est une vérification demandée.

4. TON RAPPORT — écris-le dans /tmp/panel2/rapports/A1.md, en français,
