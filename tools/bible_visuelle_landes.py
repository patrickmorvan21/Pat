...
LIEUX = [
    ...
    ("La Lande", "scene_landes_borne_frontiere_affiche_a", "borne-frontiere",
     ...
     "rows of carved notches on its face, a gigantic low sun disc sitting exactly behind it so the "
     "stone cuts the disc in two"),
    ("La Lande", "scene_landes_chemin_creux_affiche_a", "chemin-creux",
     ...
     "roots hanging from their lips, one narrow blade of light falling straight down the lane from a "
     "slit of sky"),
    ("La Lande", "scene_landes_verger_noir_affiche_a", "verger-noir",
     ...
     "overhead into a pointed gothic vault, a huge sun at the vanishing point of the central alley, a "
     "tiny cross shape standing far down that alley"),
    ("La Lande", "scene_landes_tourbiere_affiche_a", "tourbiere",
     ...
     "marching across it toward a huge low sun, the stakes and their reflections forming one long line"),
    ("La Lande", "scene_landes_cercle_qui_descend_affiche_a", "cercle-qui-descend (+ -2)",
     ...
     "a huge sun disc sitting exactly in the empty centre of the ring"),
    # ————— II. LES GIBETS —————
    ("Les Gibets", "scene_landes_colline_aux_gibets_affiche_a", "colline-aux-gibets",
     "a long crest line of black gallows in single file against a gigantic setting sun, the gallows in "
     "the middle standing empty, its rope hanging straight down across the centre of the sun disc"),
    ("Les Gibets", "scene_landes_champ_des_fixes_affiche_a", "champ-des-fixes",
     ...
     "horizon, a short rope tied to the top of every post, a huge sun low between the rows"),
    ("Les Gibets", "scene_landes_moulin_sans_ailes_affiche_a", "campement (le Moulin sans Ailes)",
     ...
     "any more, alone on a ridge, its black shape dead centre in front of an enormous sun disc, one "
     "small window lit from inside"),
    ("Les Gibets", "scene_landes_maison_du_bailli_affiche_a", "chien-du-bailli (la maison)",
     ...
     "chain running from its threshold across the ground, a huge sun behind turning the house into a "
     "black tombstone"),
    # ————— III. LE HAMEAU —————
    ("Le Hameau", "scene_landes_seuil_du_hameau_affiche_a", "serment-hameau / hameau-entree",
     ...
     "hooded men standing behind it shoulder to shoulder, a white chalk cross on every door, a low sun "
     "wedged in the gap between two roofs"),
    ...
    ("Le Hameau", "scene_landes_marche_muet_affiche_a", "marche-muet",
     ...
     "trading with raised hands and closed mouths, the sun a hard disc through a round hole in the tarp"),
    ("Le Hameau", "scene_landes_tour_de_guet_affiche_a", "tour-de-guet",
     ...
     "the village and not the moor, a huge sun directly behind the broken crown"),
    # ————— IV. LE SUD —————
    ...
    ("Le Sud", "scene_landes_chemin_du_sud_affiche_a", "chemin-du-sud",
     ...
     "huge sun sinking at the vanishing point"),
    ("Le Sud", "scene_landes_falaise_aux_cordes_affiche_a", "falaise-cordes",
     "the edge of the world: a cliff falling into a bottomless black chasm, and thousands of ropes "
     "hanging out of the sky in one dense column that plunges straight down into the void, the column "
     "lit from within like a shaft of fire"),
]
...
RENCONTRES = [
    ...
    ("La Lande", "monstre_landes_epouvantail_affiche_a", "verger-noir-2",
     ...
     "head turned all the way round to look backwards, straw arms spread wide, a huge sun disc right "
     "behind the cross"),
    ("La Lande", "monstre_landes_enlises_affiche_a", "tourbiere-2",
     ...
     "reaching up toward a low sun, fingers open, only the arms and never the bodies"),
    ("La Lande", "monstre_landes_meute_grise_affiche_a", "meute-grise-1 / -2, menace-retour-meute",
     ...
     "viewer, a huge low sun behind them so they are five black cut-outs with light between their legs"),
    ("La Lande", "monstre_landes_recousu_affiche_a", "menace-retour-recousu",
     ...
     "its whole body, standing in waist-high heather, patient, head tilted, a gigantic sun disc behind "
     "its head"),
    ("Les Gibets", "monstre_landes_corbeaux_du_compte_affiche_a", "colline-aux-gibets (examen)",
     ...
     "turned the same way as if counting, a huge sun behind them"),
    ("Les Gibets", "monstre_landes_pendu_mal_fixe_affiche_a", "pendu-mal-fixe",
     ...
     "down until his feet touch the ground, standing bent like a puppet, head cocked, a low sun behind"),
    ("Les Gibets", "monstre_landes_chien_du_bailli_affiche_a", "chien-du-bailli-2",
     ...
     "still, the chain taut across the frame, the sun behind the house"),
    ...
    ("Le Hameau", "monstre_landes_troupeau_affiche_a", "troupeau (ambiance)",
     ...
     "at the same instant toward the viewer, a huge sun on the crest behind them"),
    ...
    ("Les Gibets", "monstre_landes_pendu_qui_parle_affiche_a", "pendu-qui-parle",
     ...
     "speaking, a heavy iron seal on his chest, the taut rope rising into a colossal sun disc"),
    ("La Lande", "monstre_landes_hesitant_affiche_a", "hesitant-1..3",
     ...
     "foot raised and frozen mid-step, the low sun ahead of him"),
    ...
    ("La Lande", "monstre_landes_epoux_affiche_a", "epoux-1..3",
     ...
     "planted in the earth between them, a small fresh grave mound at their feet, a huge sun between "
     "their heads"),
    ...
    ("Les Gibets", "monstre_landes_fossoyeur_affiche_a", "champ-des-fixes-2",
     ...
     "posts on his shoulder, the sun low behind"),
    ...
    ("Le Hameau", "monstre_landes_sonneur_sans_cloche_affiche_a", "hameau (le Sonneur)",
     ...
     "vanishing into the dark opening, the sun framed in the empty arch"),
    ("Le Hameau", "monstre_landes_gamin_des_murets_affiche_a", "hameau (le Gamin)",
     ...
     "running in lines behind him, the sun behind"),
    ("Le Sud", "monstre_landes_veilleur_affiche_a", "veilleur-1..3",
     ...
     "lantern held out over the village side, the blinding southern light behind him"),
]
...
def main() -> int:
    ...
    L = [
        ...
        "noir, une figure géométrique, la lumière dans l'image, une échelle écrasante et quelque chose "
        ...
        "2. **Une silhouette minuscule, de dos, sur les lieux.** C'est ce qui donne l'échelle à toutes "
        "les références. Jamais de visage. (Les Salines gardent leur vue à la première personne.)",
        "",
