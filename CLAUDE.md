...
- **Repasse 1925:614** : (1) l'en-tête n'a PLUS de titre — seule l'icône menu (32×32, 3×3 points, fond charbon 80%) flotte en haut à droite, le fil défile dessous (`chapterLabel` n'est plus rendu) ; (2) **l'illustration de Geryon dans la maquette est le démon assis au trône** (node 1938:3672 « Property 1=1 »security du board Rencontre — le nom de calque « dithering-aldenhar (8) 1 » est un reliquat de fill remplacé, ne pas s'y fier) → `monstre_geryon.png` remplacé par ce visuel ; la gueule aux crocs (V19) reste dispo sur le board si Patrick préfère.
